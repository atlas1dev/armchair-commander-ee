using System.Collections;// Bunker +75 hp Launcher -25 hp
...
public class Controller : MonoBehaviour {
	...
	public InputField cityNameInput, unitNameInput, countryOverrideInput, manpowerInput, industryInput, fuelInput, nuke1Input, nuke2Input, aiDetectionInput, maxGeneralsInput, roundLimitInput,
	...
	public Toggle deleteToggle, moveToggle, diplomacyToggle, productionToggle, overrideTechToggle, advancedDiplomacy, reuploadToggle, lessResourcesToggle, showFlagToggle;
	...
	IEnumerator MyStart() {
		...
		CheckCountryData();
		adipon = mapData.advancedDip[myPlayerPrefs.GetInt("level")];
		producon = mapData.disableProduction[myPlayerPrefs.GetInt("level")];
		...
		if (saveFil == null || myPlayerPrefs.GetInt("custom") == 1) {
			...
			if (!editMode || myPlayerPrefs.GetInt("custom") == 1) {
				foreach (Vector2 i in mapData.cityCoordinates[mapLevel]) {
					...
					insItem.GetComponent<City>().isCapital = mapData.cityCapitals[mapLevel][index];
			...
			foreach (Vector2 i in mapData.cityCoordinates[saveFil.map]) {
				...
				insItem.GetComponent<City>().isCapital = mapData.cityCapitals[saveFil.map][index];
		...
		if (editMode) {
			try {
				...
				productionToggle.isOn = mapData.disableProduction[myPlayerPrefs.GetInt("level")];
				overrideTechToggle.isOn = mapData.overridePlayerTech[myPlayerPrefs.GetInt("level")];
				advancedDiplomacy.isOn = mapData.advancedDip[myPlayerPrefs.GetInt("level")];
	...
	public Unit spawnSoldier(Vector2 coordinates, string country, int isAxis, int troopType, int veterency,
		int tier, bool isInitial, string general = "", int generalLevel = 0, string defaultGeneral = "", bool flippedHorizontal = false, string unitName = "") {
		if (coordinates.y < 0f) {
		...
		if (unitName == "") {
			if (i.troopType == Troop.fortification) {
				i.unitName = "Fortification";
			} else if (i.troopType == Troop.infantry) {
				i.unitName = "Infantry";
			} else if (i.troopType == Troop.armor) {
				i.unitName = "Armored";
			} else if (i.troopType == Troop.artillery) {
				i.unitName = "Artillery";
			} else if (i.troopType == Troop.navy) {
				i.unitName = "Naval";
			}
		} else {
			i.unitName = unitName;
				...
				if (notMultiplayer || countriesIsAxis[i.currentTile.country] == countriesIsAxis[playerCountry] || isHost && countriesIsAxis[i.currentTile.country] != opponentTeam) {
					...
					if (i.currentTile.occupant != null) {
						...
						if (i.currentTile.occupant.troopType == Troop.infantry) {
							...
							if (i.isCapital && i.currentTile.radiationLeft < 1) {
								i.currentTile.occupant.health += 10f;
							}
						} else {
					...
					if (((playerCountry != i.currentTile.country || deltaRoundAuto) && i.currentTile.radiationLeft < 1 && playerIsAxis == countriesIsAxis[i.currentTile.country] ||
						...
						if (i.currentTile.radiationLeft < 1) {
					...
					if (i.isCapital && !noReducedResources && i.currentTile.radiationLeft < 1) {
						countryDatas[i.currentTile.country].manpower += 10;
						countryDatas[i.currentTile.country].industry += 10;
					}
				}
			}

			foreach (Tile i in oilRigs) {
				if (i && i.radiationLeft < 1 && (notMultiplayer || countriesIsAxis[i.country] == countriesIsAxis[playerCountry] || isHost && countriesIsAxis[i.country] != opponentTeam)) {
		...
		foreach (City i in cities) {
			try {
				if (i != null && i.currentTile != null && (myPlayerPrefs.GetInt("multiplayer") == 0 ||
					...
					&& !countriesIsNeutral.Contains(i.currentTile.country) &&
					countryDatas[i.currentTile.country].industry >= 15 && countryDatas[i.currentTile.country].fuel > 10 && i.airportTier > 0) {
					...
					int affordable = 0;
					if (countryDatas[i.currentTile.country].industry >= 35 && countryDatas[i.currentTile.country].fuel >= 30 ) {
						affordable = 4;
					} else if (countryDatas[i.currentTile.country].industry >= 25 && countryDatas[i.currentTile.country].fuel >= 20 ) {
						affordable = 2;
					} else if (countryDatas[i.currentTile.country].industry >= 20 && countryDatas[i.currentTile.country].fuel >= 15 ) {
						affordable = 1;
					}

					if (bestTarget != null) {
						if (i.airportTier >= 3) {
							aiAirStrike(i, i.currentTile.country, bestTarget, affordable);
						} else if (i.airportTier == 2) {
							if (affordable != 4) {
								aiAirStrike(i, i.currentTile.country, bestTarget, affordable);
							} else {
								aiAirStrike(i, i.currentTile.country, bestTarget, 2);
							}
						} else if (i.airportTier == 1) {
							if (affordable != 4 && affordable != 2) {
								aiAirStrike(i, i.currentTile.country, bestTarget, affordable);
							} else {
								aiAirStrike(i, i.currentTile.country, bestTarget, 1);
							}
						}
					}
				}
	...
	private void AirStrikeDelay(City sender, Tile target) {
		...
		if (airplaneType == 4 && nuclearWarheadDropdown.value > 0) {
			switch (nuclearWarheadDropdown.value) {
				case 1:
					...
					break;
				case 2:
					damage = 1150 * UnityEngine.Random.Range(0.9f, 1.1f); //add collateral damage
					collat = true;
					isNuke = true;
					break;
			}
		...
		if (target.occupant) {
			...
			StartCoroutine(EnableSmoke(0.8f, target, 2));
	...
	IEnumerator EnableSmoke(float delay, Tile t, int falloutAmount) {
        for (float i = 0; i < delay; i += Time.deltaTime) {
            yield return null;
        }
        t.smokeLeft = falloutAmount;
    }
	IEnumerator airStrikeDelay(City sender, City target, float damage = -1, bool isCollat = false) {
		...
		if (!isCollat)
			ad = CheckAirDefences(sender.currentTile, target.currentTile, true);
		...
		if (ad <= 0f) {
			...
			insItem.GetComponent<Text>().text = "Destroyed";
		}
		inAirStrike = false;
	}
	...
	public float CalculateTerrainGeneralPerksModifier(Unit sender) {
		...
		} else if ((sender.currentTile.terrain == Terrain.city || sender.currentTile.terrain == Terrain.village) && playerData.generals[sender.general].ContainsPerk(General.GeneralPerk.City)) {
	...
	public bool mouseInUI() {
		...
		if (editMode && (editCategory == 13 || editCategory == 16)) {
			RectTransform i = editCategory == 13 ? editModeHideUI[0] : editModeEventUI[0]; //first item background
	...
	public void CheckInfo() {
		...
		p.moveText.text = ((int)chosenUnit.movement).ToString();
		p.unitNameText.text = chosenUnit.unitName;
	}
	...
	public void DeleteAll(Text button) {
		if (deleteToggle.isOn || editCategoryDropdown.value == 1) {
			...
			switch (editCategoryDropdown.value) {
				case 1: //all tiles
					foreach (Tile i in tiles.Values) {
						if (i != null && i.terrain != Terrain.water) {
							i.terrain = Terrain.plains;
							i.updateTerrain();
						}
					}
					break;
	...
	public void onEditCategoryChanged() {
		...
		if (editCategory == 13) {
			if (hideEditUI)
				toggleHideUI();
			if (!hideEventUI && editCategory != 16) {
				toggleEventUI();
			}
		} else {
			if (editCategory == 16 && hideEventUI || !hideEventUI && editCategory != 16) {
				toggleEventUI();
				EventCitiesChanged(true);
				UpdateEventDropdown();
				OnEventChanged();
		...
		if (editCategory != 0 && editCategory != 1 && editCategory != 4 && editCategory != 5 && editCategory != 12) {
	...
	public void UpdateEventSelections() {
		try {
			if (events.Count > 0 && editCategory == 16) {
	...
	void Update() {
		...
		if (!canBuild && selectedCity != null && !airplaneMode) {
			if (selectedTile.occupant == null && !producon || selectedCity.airportTier >= 1 || selectedCity.nuclearTier >= 1) {
				buildButtonOn = true;
				canBuild = true;
			}
		}
		if (selectedBuildTile != null && !producon) {
			buildButtonOn = true;
		}
		if (Input.GetMouseButtonDown(0))
		...
		if (editMode && hideEditUI && !mouseInUI() && !paused) { // MAP EDITOR CATEGORY 1
			List<string> countries = new List<string>(countriesIsAxis.Keys);
			...
			if (Input.GetMouseButton(0) && !mouseInUI() && !paused) {
				...
				if (hit.collider != null && hit.collider.GetComponent<Tile>() != null) {
					...
					} else if (editCategory == 17) {
						if (t.occupant != null) {
							t.occupant.isLocked = !deleteToggle.isOn;
						}
					} else if (editCategory == 18) {
						if (t.city != null && !t.city.isPort) {
							bool hasCapital = false;
							foreach (City i in cities) {
								if ((i.country == t.country) && i.isCapital) {
									hasCapital = true;
									break;
								}
							}

							if (!hasCapital && !deleteToggle.isOn) {
								t.city.isCapital = true;
							} else if (deleteToggle.isOn) {
				...
				if (airplaneMode) {
					...
					if (!inAirStrike) {
						if (countryDatas[playerCountry].fuel >= productBar.airplaneOilCost && countryDatas[playerCountry].industry >= productBar.airplaneIndustryCost && hit.collider != null &&
							...
							if (airplaneType != 0) {
								...
								AirplaneAnimator ins = Instantiate(productBar.airplanes[airplaneType], new Vector3(hit.collider.transform.position.x + -0.9f, hit.collider.transform.position.y + 0.70f, -350f), productBar.airplanes[airplaneType].transform.rotation).GetComponent<AirplaneAnimator>();
								ins.targetTile = hit.collider.GetComponent<Tile>();
							}
							inAirStrike = true;
					...
					if (hit.collider != null && hit.collider.GetComponent<Tile>() != null) {
						...
						if (myTile.movable && myTile.occupant == null && (!myTile.isCity || countriesIsAxis[myTile.country] == playerIsAxis || myTile.city.health <= 0f)) {
							if (myPlayerPrefs.GetInt("animationsOn") == 0) {
								selectedTile.occupant.moveToDestination(selectedTile.findPathToDestination(myTile), false, selectedTile);
							} else {
								selectedTile.occupant.moveToDestination(selectedTile.findPathToDestination(myTile), true, selectedTile);
							}
						...
						if (myTile.isCity && (!canBuild || selectedCity != myTile.city) && (myTile.occupant == null && !producon || myTile.city.airportTier >= 1) &&
							(myTile.country == playerCountry || myTile.occupant != null && myTile.occupant.country == playerCountry)) {// sus nuclear
							selectedCity = myTile.city;
							buildButtonOn = true;
							canBuild = true;
							if (selectedSoldier != null)
