...
public class IngamePopup : MonoBehaviour {
	...
	public GameObject hostingPopup, readCustomMapPopup;
	...
	public void SaveCustomMapFile()
    {
        File.WriteAllText(Path.Combine(Application.persistentDataPath, "CustomMaps", mapSelectDropdown.options[mapSelectDropdown.value].text + ".txt"), myPlayerPrefs.GetString("customData" + dropdownValues[mapSelectDropdown.value]));
	}

    public void LoadReadCustomMapPopup()
    {
        Transform insItem = Instantiate(readCustomMapPopup, GameObject.Find("Canvas").transform).transform;
        insItem.position = new Vector2(Screen.width / 2, Screen.height / 2);
    }
	public void likeMap() {
		if (!liking)
	...
	void RemoveMapFromAllList(int mapId) {
		...
		foreach (string j in myPlayerPrefs.GetString("customDataAll").Split(',')) {
			try {
				...
				print("map not parsed properly: " + j + " " + e);
			}
		}
