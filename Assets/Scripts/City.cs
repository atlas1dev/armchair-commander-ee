...
public class City : MonoBehaviour {
    ...
    [SerializeField]
    private Image flagImage, cityImage, factoryImage, airportImage, nuclearImage, strategicImage, capitalImage, cityShield;
    [SerializeField]
    ...
    public bool isStrategic, isPort, isCapital;
    ...
    public void Start() {
        ...
        if (defenceTier > 0) {
            ...
            capitalImage.transform.localPosition = new Vector2(0.25f, 1.42f);
        } else {
            healthDisplay.SetActive(false);
            strategicImage.transform.localPosition = new Vector2(0.25f, 1.133f);
            capitalImage.transform.localPosition = new Vector2(0.25f, 1.133f);
        }
    ...
    void Update() {
        ...
        if (capitalImage.enabled != isCapital) {
            capitalImage.enabled = isCapital;
        }
        if (isStrategic && isCapital) {
            strategicImage.transform.localPosition = new Vector2(-0.09f, strategicImage.transform.localPosition.y);
            capitalImage.transform.localPosition = new Vector2(0.5f, capitalImage.transform.localPosition.y);
        } else if (isStrategic) {
            strategicImage.transform.localPosition = new Vector2(0.249f, strategicImage.transform.localPosition.y);
        } else if (isCapital) {
            capitalImage.transform.localPosition = new Vector2(0.249f, capitalImage.transform.localPosition.y);
        }
        if (cityName != "") {
        ...
        if (Controller.instance.editMode) {
            if (defenceTier > 0 && !healthDisplay.activeInHierarchy) {
                ...
                capitalImage.transform.localPosition = new Vector2(0.25f, 1.375f);
