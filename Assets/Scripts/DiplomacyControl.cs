	...
	void Start() {
		...
		foreach (CountryData i in countriesAvailable) {
			...
			if (!controller.countriesIsNeutral.Contains(controller.playerCountry) && controller.countriesIsAxis[i.name] != controller.playerIsAxis && !controller.countriesIsNeutral.Contains(i.name)) {
				...
				insItem.GetComponent<Image>().color = Color.red;
				...
				insItem.countryNameDisplay.text += " " + CustomFunctions.TranslateText("(Allied)");
				insItem.GetComponent<Image>().color = Color.green;

				if (controller.countryDatas[controller.playerCountry].nukes[0] != 0 || controller.countryDatas[controller.playerCountry].nukes[1] != 0) {
					insItem.nukeAid.interactable = true;
				}
			}
			index++;
		}
