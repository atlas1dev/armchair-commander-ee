...
public class MapInfo {
	...
	public bool[] cityStrategics, soldierIsStrategics, cityCapitals;
	public int[] cityDefenceTiers; //optional NEW FEATURE
	public string[] unitNames, soldierGenerals;
	...
	public bool allowReupload, disableDiplomacy, disableProduction, overridePlayerTech, advancedDip, lessResources;
	...
	public MapInfo(Controller controller) {
		...
		disableProduction = controller.productionToggle.isOn;
		advancedDip = controller.advancedDiplomacy.isOn;
		...
		cityCapitals = new bool[controller.cities.Count];
		for (i = 0; i < controller.cities.Count; i++)
			cityCapitals[i] = controller.cities[i].isCapital;
		...
		unitNames = new string[controller.soldiers.Count];
		for (i = 0; i < controller.soldiers.Count; i++)
			unitNames[i] = controller.soldiers[i].unitName;
...
public class MapData : MonoBehaviour {
	...
	public string[][] cityNames, countryCoordinateNames, unitNames, soldierGenerals;
	public bool[][] cityStrategics, soldierIsStrategics, cityCapitals, soldierIsLocked;
	[HideInInspector]
	public bool[] disableDiplomacy, disableProduction, overridePlayerTech, advancedDip, allowReupload, lessResources;
	//[HideInInspector]
	...
	void Awake() {
		...
		advancedDip = new bool[mapInfos.Length];
		for (int i = 0; i < mapInfos.Length; i++) {
			//try {
			advancedDip[i] = mapInfos[i].advancedDip;
			//} catch {
			//	advancedDip[i] = false;
			//	print("no tech");
			//}
		...
		disableProduction = new bool[mapInfos.Length];

		for (int i = 0; i < mapInfos.Length; i++) {
			disableProduction[i] = mapInfos[i].disableProduction;
		}
		...
		unitNames = new string[mapInfos.Length][];
		for (int i = 0; i < mapInfos.Length; i++)
			unitNames[i] = mapInfos[i].unitNames;

		cityCapitals = new bool[mapInfos.Length][];
		for (int i = 0; i < mapInfos.Length; i++) {
			if (mapInfos[i].cityCapitals == null)
				cityCapitals[i] = new bool[mapInfos[i].cityNames.Length];
			else {
				cityCapitals[i] = mapInfos[i].cityCapitals;
			}
		}
